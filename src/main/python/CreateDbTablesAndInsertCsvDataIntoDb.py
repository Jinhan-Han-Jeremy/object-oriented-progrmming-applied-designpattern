...
def insert_member_csv_to_mysql(csv_file, connection):
    # CSV 파일을 읽을 때, UTF-8 인코딩으로 처리
    data = pd.read_csv(csv_file, encoding='utf-8')

    # NaN 값을 None으로 변환 (pandas의 NaN 값을 DB의 NULL로 처리)
    data = data.where(pd.notnull(data), None)

    insert_query = """
    INSERT INTO team_member (name, role, level, state, skills_performance)
    VALUES (%s, %s, %s, %s, %s)
    """
    ...
    for _, row in data.iterrows():
        # skills_performance가 JSON 형식일 경우, 이를 처리
        skills_performance = None
        if row['skills_performance']:
            try:
                # JSON 데이터 파싱
                skills_performance = json.loads(row['skills_performance'])
                # JSON 데이터를 깨끗한 문자열로 변환
                skills_performance = json.dumps(skills_performance, ensure_ascii=False)
            except json.JSONDecodeError as e:
                # JSON 파싱 오류 발생 시 오류와 해당 데이터를 출력
                print(f"JSONDecodeError: {e} - Invalid JSON data: {row['skills_performance']}")
                continue  # 문제가 있는 행은 건너뛰고 다음으로 넘어감
...
def main():
    ...
    if connection is not None:
        ...
        clear_table(connection, 'team_member')
        ...
        tasks_team_member_csv_file = 'C:\\Users\\USER\\Downloads\\team_member.csv'
        ...
        insert_member_csv_to_mysql(tasks_team_member_csv_file, connection)
